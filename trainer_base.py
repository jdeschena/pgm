...
class TrainerBase(L.LightningModule):
  def __init__(
    ...
    elif self.config.algo.backbone == 'encoder-decoder':
      self.backbone = models.encoder_decoder.PartitionDIT(
        self.config, vocab_size=self.vocab_size)
    else:
      raise ValueError(self.config.algo.backbone)
  ...
  def _validate_configuration(self):
    assert self.config.algo.backbone in {
      'dit', 'hf_dit', 'partition-dit', 'partition-dit-orig', 
      'encoder-decoder'}, self.config.algo.backbone
...
class AbsorbingState(Diffusion):
  ...
  def _ddpm_update_efficient(self, x, t, dt, p_x0, 
    clean_positions, noisy_positions, concrete_lengths):
    _, alpha_t = self.noise(t)
    assert alpha_t.ndim == 2
    if p_x0 is None:
      log_p_x0 = self.forward(x, self._sigma_from_alphat(alpha_t), 
    ...
    # Always denoise the selected positions
    sampled_x0 = sample_categorical(p_x0)
    return sampled_x0

  def _ddpm_update(self, x, t, dt, p_x0=None, group_idxs=None,
    ...
    # Updated code: equivalent to the original mdlm code
    #  but will be more suited to be adapted for the 
    ...
    # Denoise only if the token is currently masked AND the random draw succeeds
    should_denoise_mask = is_masked & should_denoise_draw
    _x = torch.where(should_denoise_mask, sampled_x0, x)
    
    # Group 0 is considered unmasked
    if group_idxs is not None:
      ...
      out = torch.where(x != self.mask_index, x, _x)
      return p_x0, out
  ...
  def _analytic_update(self, x, t, dt, group_idxs=None):
    ...
    if group_idxs is None:
      ...
      group_idxs = torch.where(out == x, 
                               group_idxs, 
                               torch.logical_not(group_idxs))
      return out, group_idxs

  def _denoiser_update(self, x, t, group_idxs):
    sigma = self._sigma_from_alphat(self.noise(t)[1])
    score = self._get_score(x, sigma, group_idxs)
    if self.config.sampling.use_float64:
      score = score.to(torch.float64)
    ...
    if group_idxs is None:
      ...
      return torch.where(group_idxs == 0, x, _x)
