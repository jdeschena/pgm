...
class MultiInceptionMetrics(Metric):
    ...
    def update(self, images, image_type="fake") -> None:
        ...
        features = self.gather_and_concat(features).cpu()
        logits = self.gather_and_concat(logits).cpu()
    ...
    def get_kth_value(self, unsorted, k, axis=-1):
        print('unsorted', unsorted.shape, k)
    ...
    def compute(self) -> dict:
        ...
        print(f'Num. real features: {len(real_features)}')
        print(f'Num. fake features: {len(fake_features)}')

        real_features = real_features[:50_000]
        fake_features = fake_features[:50_000]
        
        assert len(real_features) == len(fake_features), f'{real_features.shape=}, {fake_features.shape=}'
    ...
    def gather_and_concat(tensor):
        ...
        Uses a two-stage approach: first exchange sizes, then exchange padded tensors.
        ...
        """
        if tensor is None:
            raise ValueError("tensor cannot be None in gather_and_concat")
            
        if dist.is_available() and dist.is_initialized() and dist.get_world_size() > 1:
            world_size = dist.get_world_size()
            
            # Stage 1: Exchange tensor sizes across all processes
            local_size = torch.tensor([tensor.shape[0]], device=tensor.device, dtype=torch.long)
            size_list = [torch.zeros_like(local_size) for _ in range(world_size)]
            dist.all_gather(size_list, local_size)
            
            # Convert to list of integers and find max size
            sizes = [size_tensor.item() for size_tensor in size_list]
            max_size = max(sizes)
            assert max_size > 0

            if tensor.shape[0] < max_size:  # pad
                padding_shape = [max_size - tensor.size(0)] + list(tensor.shape[1:])
            ...
            trimmed_tensors = [tensor[:l] 
                for l, tensor in zip(sizes, gathered_tensors) 
                if l > 0]
            concatenated_tensor = torch.cat(trimmed_tensors, dim=0)
