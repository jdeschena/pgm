...
if __name__ == '__main__':
  config = dict(model=dict(
    ...
    algo=dict(causal_attention=False),
    )
  ...
  # Check training mode forward
  seed = 435
  torch.manual_seed(seed)
  torch.cuda.manual_seed(seed)
  torch.cuda.manual_seed_all(seed)
  model = PartitionDIT(config, 50258)
  ckpt_name = '30-505000.ckpt'
  ckpt = torch.load(ckpt_name, map_location='cpu', weights_only=False)
  mapped_ckpt = dict()
  for k, v in ckpt['state_dict'].items():
    k = k.replace('backbone.', '')
    mapped_ckpt[k] = v
  #model.load_state_dict(mapped_ckpt)
  #print('Loaded checkpoint')
  model.cuda()
  # generator = torch.Generator(device='cuda').manual_seed(0)
  # for param in model.parameters():
  #   param.data.add_(torch.randn(param.data.shape, device='cuda', generator=generator) * 1e-3)
  
  """
  x = torch.randn(BS, L, H).cuda()
  sigma = torch.rand(BS).cuda()
  group_idxs = torch.randint(0, 2, size=(BS, L)).cuda()
  out = model(x, sigma, group_idxs)
  print("Training path working!")

  clean_positions0 = [0, 10, 23, 32]
  clean_positions1 = [1, 2]

  noisy_positions0 = list(set(range(L)) - set(clean_positions0))
  noisy_positions1 = list(set(range(L)) - set(clean_positions1))

  clean_positions = [clean_positions0, clean_positions1 + [0] * 2]
  noisy_positions = [noisy_positions0 + [0] * 2, noisy_positions1]
  clean_positions = torch.tensor(clean_positions).cuda()
  noisy_positions = torch.tensor(noisy_positions).cuda()
  concrete_lengths = torch.tensor([4, 2]).cuda()
  sigma = torch.rand(2).cuda()

  x = torch.randint_like(clean_positions, 100)
  out = model(x, sigma, clean_positions=clean_positions, noisy_positions=noisy_positions, concrete_lengths=concrete_lengths, use_inference_mode=True)
  print("Forward done!")
  out.sum().backward()
  print("Backward done!")

  """
  # Check that inference mode and training mode return the same 
  model.eval()
  x = torch.tensor([[0,1,2,3,4] + [0] * (config.model.length - 5)]).cuda()
  sigma = torch.zeros(size=(1,)).cuda()
  group_idxs = torch.tensor([[0,0,0,0,0] + [1] * (config.model.length - 5)]).cuda()
  out_naive = model(x, sigma, group_idxs)
  out_naive = out_naive[:, :5, :]

  x_perm = [1,0,3,2,4]
  #x_perm = [4,0,1,3,2]
  x_perm_inv = [1,0,3,2,4]
  #x_perm_inv = [1,2,4,3,0]
  x = torch.tensor([x_perm]).cuda()
  clean_positions = torch.tensor([x_perm]).cuda()
  noisy_positions = torch.tensor([[5,6,7]]).cuda()
  concrete_lengths = torch.tensor([5]).cuda()
  out_efficient = model(x, sigma, clean_positions=clean_positions, 
                        noisy_positions=noisy_positions, 
                        concrete_lengths=concrete_lengths, 
                        use_inference_mode=True)
  
  embed_inference = torch.load('inference_embed.pth')
  embed_train = torch.load('train_embed.pth')
  if not torch.allclose(embed_inference[:, x_perm_inv], embed_train[:, :5]):
    diff = (embed_train[:, :5] - embed_inference[:, x_perm_inv]).abs()
    raise ValueError(f'Error in embeddings: {diff.max().item()}, {diff.mean().item()}')
  else:
    print('Embeddings match')

  layer0_inference = torch.load("layer0_inference.pth")
  layer0_train = torch.load("layer0_train.pth")

  if layer0_inference.shape == layer0_train.shape:
    if not torch.allclose(layer0_inference, layer0_train):
      diff = (layer0_inference - layer0_train).abs()
      #raise ValueError(f'Error in layer0 (match shape): {diff.max().item()}, {diff.mean().item()}')
    else:
      print('Layer0 (matching shapes) are equal')
  else:
    print("TRYING WITH EQUAL!!!")
    print(torch.equal(layer0_inference[:, x_perm_inv], layer0_train[:, :5]))
    if not torch.allclose(layer0_inference[:, x_perm_inv], layer0_train[:, :5]):
      diff = (layer0_inference[:, x_perm_inv] - layer0_train[:, :5]).abs()
      #raise ValueError(f'Error in layer0: {diff.max().item()}, {diff.mean().item()}')
    else:
      print('Layer0 match')
  inference_encoder_input = torch.load("inference_mode_encoder_input.pth")
  inference_encoder_input = inference_encoder_input[:, x_perm_inv, :]
  train_encoder_input = torch.load("train_mode_encoder_input.pth")[:, :5, :]

  if not torch.allclose(inference_encoder_input, train_encoder_input):
    diff = torch.abs(inference_encoder_input - train_encoder_input)
    #raise ValueError(f'encoder input differs: {diff.max().item()}, {diff.mean().item()}')
  else:
    print('encoder input matches.')

  inference_mode_tensor = torch.load("inference_mode_encoder_output.pth")
  inference_mode_tensor = inference_mode_tensor[:, x_perm_inv, :]
  train_mode_tensor = torch.load("train_mode_encoder_output.pth")[:, :5, :]

  if not torch.allclose(inference_mode_tensor, train_mode_tensor):
    diff = torch.abs(inference_mode_tensor - train_mode_tensor)
    #raise ValueError(f'Encoder output differs: {diff.max().item()}, {diff.mean().item()}')
  else:
    print('encoder output is correct')  

  group_swap_train_output = torch.load("train_mode_group_swap_output.pth")
  group_swap_inference_output = torch.load("inference_mode_group_swap_output.pth")
  if not torch.allclose(group_swap_inference_output, group_swap_train_output[:, 5:8]):
    max_diff = torch.abs(group_swap_inference_output - group_swap_train_output[:, 5:8]).max()
    mean_diff = torch.abs(group_swap_inference_output - group_swap_train_output[:, 5:8]).mean()
    #raise ValueError(f'GroupSwap output differs: {max_diff.item()}, {mean_diff.item()}')
  else:
    print("GroupSwap output matches")
  
  inference_decoder_output = torch.load("inference_mode_decoder_output.pth")
  train_mode_decoder_output = torch.load("train_mode_decoder_output.pth")
  #print(inference_decoder_output.shape)
  #print(train_mode_decoder_output.shape)
  #inference_decoder_output = inference_decoder_output[:, x_perm_inv, :]

  if not torch.allclose(inference_decoder_output, train_mode_decoder_output[:, 5:8]):
    max_diff = torch.abs(inference_decoder_output - train_mode_decoder_output[:, 5:8]).max()
    mean_diff = torch.abs(inference_decoder_output - train_mode_decoder_output[:, 5:8]).mean()
    print("Decoder output differs")
    print(max_diff)
    print(mean_diff)
  else:
